...
using libMultiRobotPlanning::AG_ECBS;
...
int main(int argc, char ** argv)
{
    ...
    float w = 1.25;  // ECBS suboptimality weight
    ...
    desc.add_options()
        ...
        ("weight,w", po::value<float>(&w)->default_value(1.25), "sub-optimality weight (for ECBS)")
    ...
    try {
        ...
        std::cerr << "Error loading input file: " << inputFile << " (" << e.what() << ")" << std::endl;
        return 1;
    }
    ...
    for (const auto& node : config["map"]["obstacles"]) {
        obstacles.insert(Location(node[1].as<int>(), node[0].as<int>()));
    }
    ...
    AG_ECBS<State, Action, float, Conflict, Constraints, Environment> cbs(mapf, w);
